...
Blockly.JavaScript['histogram'] = function(block) {
  ...
  if(number_hist_class<0)number_hist_class*=-1;

  var x_range_min = 'Math.min.apply(null,' + data_hist + ') - (Math.min.apply(null,' + data_hist + ')%'+number_hist_class+')';
  var x_range_max = 'Math.max.apply(null,' + data_hist + ') + ('+number_hist_class+'-(Math.max.apply(null,' + data_hist + ')%'+number_hist_class+'))';
  var x_range_bool = new Boolean(false);
  var hist_low_bool = new Boolean(false);
  var hist_high_bool = new Boolean(false);
  var x_low_bool =new Boolean(false);

  var x_surplus = 0;
  var x_max_high_check=0;
  //( value_hist_low=="" && value_hist_high=="" ) ? x_range_bool = true : x_range_bool = false;
  //x_range_min = +x_range_min+' - 10';
  if ( value_hist_low == "" ) {
    value_hist_low = x_range_min;
    hist_low_bool = true;
  }
  if ( value_hist_high == "" ){
    value_hist_high = x_range_max;
    hist_high_bool = true;
  }
  if(value_hist_low>x_range_min)x_low_bool=true;
  var xbins_start = 0;
  var xbins_end = 500;
  if ((value_hist_low%number_hist_class!=0)&&(hist_low_bool==false)){
    xbins_start=value_hist_low;
    xbins_end=value_hist_high
    //if(xbins_start%number_hist_class!=0){
    if(hist_high_bool==true){  
      x_surplus = xbins_start%number_hist_class;
      //x_max_high_check = xbins_end%number_hist_class;
      
      x_range_max = 'Math.max.apply(null,' + data_hist + ')+('+number_hist_class+'-((Math.max.apply(null,' + data_hist + ')-'+x_surplus+')%'+number_hist_class+'))';

      var x_max='Math.max.apply(null,'+data_hist+')-'+x_surplus;
      value_hist_high = x_range_max;
      
      
      /*if(x_surplus!=x_max_high_check){
        //value_hist_high+=1;
        value_hist_high=''+value_hist_high+'-'+x_max_high_check+'';
      }*/
    }
    //}
    //value_hist_high=''+value_hist_high+'-'+number_hist_class+'+1';
  ...
  var str = "trace1 = {marker:{line:{width:1}}, xbins:{start:"+ xbins_start +",size:"+ number_hist_class +"}"
